...
class MemberCount_DatabaseFunctions {
  ...
  get_Guild_Ids_MemberCount_ChannelId() {
    return new Promise(function (resolve, reject) {
      ...
      db.all(
        "SELECT CAST(Guild_Id as TEXT) as Guild_Id FROM MemberCount_ChannelId",
        [],
        function (err, rows) {
          db.close();
          if (err) {
  ...
  get_Guild_Ids_MemberCount_Collection() {
    return new Promise(function (resolve, reject) {
      const db = new sqlite3.Database("./lib/database/SQLite.db");
      db.all(
        "SELECT CAST(Guild_Id as TEXT) as Guild_Id FROM Member_Count_Collection",
        [],
        function (err, rows) {
          db.close();
          if (err) {
  ...
  get_MemberCount_ChannelId(Guild_Id) {
    return new Promise(function (resolve, reject) {
      const db = new sqlite3.Database("./lib/database/SQLite.db");
      db.all(
        "SELECT * FROM MemberCount_ChannelId WHERE Guild_Id = ?",
        [Guild_Id],
        function (err, rows) {
          db.close();
          if (err) {
  ...
  get_MemberCount_SelectValue(Guild_Id) {
    return new Promise(function (resolve, reject) {
      const db = new sqlite3.Database("./lib/database/SQLite.db");
      db.all(
        "SELECT Menu_Select_Values FROM MemberCount_ChannelId WHERE Guild_Id = ?",
        [Guild_Id],
        function (err, rows) {
          db.close();
          if (err) {
  ...
  update_Member_Count_Database(
    ...
    Bots_Count,
    All_Online_Count,
    All_Online_Count_include_idle,
    All_Online_Count_include_idle_dnd,
    All_Offline_Count,
    User_Online_Count,
    User_Online_Count_include_idle,
    User_Online_Count_include_idle_dnd,
    User_Idle_Count,
    User_Dnd_Count,
    User_Offline_Count
  ) {
    // 用Promise来获取Guild_Ids的遞歸
    this.get_Guild_Ids_MemberCount_Collection()
      .then(function (Guild_Ids) {
        ...
        if (Guild_Ids.includes(guildId.toString())) {
          db.serialize(function () {
            db.run(
              "UPDATE Member_Count_Collection SET All_Members_Count = ?, All_Online_Count_include_idle = ?, All_Online_Count_include_idle_dnd = ?, Users_Count = ?, Bots_Count = ?," +
          ...
          db.serialize(function () {
            db.run(
              "INSERT INTO Member_Count_Collection VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
              [
                guildId,
                All_Members_Count,
                Users_Count,
  ...
  insert_MemberCount_ChannelId(
    Guild_Id,
    Category_Id,
    All_Members_Count_Id,
    Users_Count_Id,
    Bots_Count_Id,
    All_Online_Count_Id,
    All_Online_Count_include_idle_Id,
    All_Online_Count_include_idle_dnd_Id,
    All_Offline_Count_Id,
    User_Online_Count_Id,
    User_Online_Count_include_idle_Id,
    User_Online_Count_include_idle_dnd_Id,
    User_Idle_Count_Id,
    User_Dnd_Count_Id,
    User_Offline_Count_Id,
    Added_User_Id,
    Added_dateTime,
    Menu_Select_Values
  ) {
    ...
    db.serialize(function () {
      db.run(
        "INSERT INTO MemberCount_ChannelId VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        [
          Guild_Id,
          Category_Id,
          All_Members_Count_Id,
  ...
  delete_MemberCount_ChannelId(Guild_Id) {
    ...
    db.serialize(function () {
      db.run(
        "DELETE FROM MemberCount_ChannelId WHERE Guild_Id = ?",
        [Guild_Id],
        function (err) {
          if (err) {
  ...
  delete_MemberCount_Collection(Guild_Id) {
    ...
    db.serialize(function () {
      db.run(
        "DELETE FROM Member_Count_Collection WHERE Guild_Id = ?",
        [Guild_Id],
        function (err) {
          if (err) {
            return console.log(`DELETION:${err.message}`);
...
export default MemberCount_DatabaseFunctions;