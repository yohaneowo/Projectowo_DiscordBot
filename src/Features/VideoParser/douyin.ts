...
const douyin_parser = async (message, regex_result) => {
  ...
  axios
    ...
    .then((response) => {
      ...
      const video_url = jsonData.video_data.nwm_url_HQ
      ...
      axios
        ...
        .then((response) => {
          const writer = fs.createWriteStream(
            "./src/lib/video/douyin_api/video.mp4"
          )
        ...
        .then(() => {
          ...
          const videoPath = "./src/lib/video/douyin_api/video.mp4"
          fs.stat(videoPath, (err, stats) => {
            ...
            if (fileSizeInMB < 25) {
              message.channel.send({
                ...
                files: ["./src/lib/video/douyin_api/video.mp4"]
              })
              message.delete()
            } else {
              message.channel.send({
