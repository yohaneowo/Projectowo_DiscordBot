...
const {
  ...
  // @ts-expect-error TS(2451): Cannot redeclare block-scoped variable 'MemberCoun... Remove this comment to see the full error message
  MemberCount_DatabaseFunctions
} = require("../../commands_modules/countstatus-displayer/cs_databaseFunctionManager.js");
const {
...
} = require("../../commands_modules/countstatus-displayer/cs_channelFunctionManager.js");
const {



  // @ts-expect-error TS(2451): Cannot redeclare block-scoped variable 'MemberCoun... Remove this comment to see the full error message
  MemberCount_Interaction_Components,



  // @ts-expect-error TS(2451): Cannot redeclare block-scoped variable 'ServerStat... Remove this comment to see the full error message
  ServerStatusIds
} = require("../../commands_modules/countstatus-displayer/cs_component.js");
const {
...
module.exports =
  ...
  async (interaction) => {
    ...
    const usersCount = interaction.guild.members.cache.filter(
    ...
    const msg = await interaction.editReply({
      content: "Select [Status] to display",
    ...
    interaction.guild.members
      ...
      .then(async (fetchedMembers) => {
        const allOnline_Count = fetchedMembers.filter(
          (member) => member.presence?.status === "online"
        ).size;
        const allOnline_Count_Idle_included = fetchedMembers.filter(
          (member) =>
            member.presence?.status === "online" ||
            member.presence?.status === "idle"
        ).size;
        const allOnline_Count_Idle_Dnd_included = fetchedMembers.filter(
          (member) =>
            member.presence?.status === "online" ||
            member.presence?.status === "idle"
        ).size;
        const allOffline_Count = fetchedMembers.filter(
          (member) => !member.presence?.status
        ).size;
        const userOnline_Count = fetchedMembers.filter(
          (member) =>
            member.presence?.status === "online" && member.user.bot == false
        ).size;
        const userOnline_Count_Idle_included = fetchedMembers.filter(
          (member) =>
            member.presence?.status === "online" ||
            (member.presence?.status === "idle" && member.user.bot == false)
        ).size;
        const userOnline_Count_Idle_Dnd_included = fetchedMembers.filter(
          (member) =>
            member.presence?.status === "online" ||
            member.presence?.status === "idle" ||
            (member.presence?.status === "dnd" && member.user.bot == false)
        ).size;
        const userIdle_Count = fetchedMembers.filter(
          (member) =>
            member.presence?.status === "idle" && member.user.bot == false
        ).size;
        const userDnd_Count = fetchedMembers.filter(
          (member) =>
            member.presence?.status === "dnd" && member.user.bot == false
        ).size;
        const userOffline_Count = fetchedMembers.filter(
        ...
        menuCollector.on("collect", async (i) => {
          ...
          if (i.user.id === interaction.user.id) {
            ...
            i.reply({
              content: "These Selection aren't for you!",
              ephemeral: true
            });
          }
        });
        ...
        Button_collector.on("collect", async (i) => {
          ...
          if (i.user.id === interaction.user.id) {
            ...
            if (isMenuSelect) {
              ...
              databaseFunctionManager
                .get_Guild_Ids_MemberCount_ChannelId()
                .then(async (Guild_Ids) => {
                  ...
                  if (!Guild_Ids.includes(guild_Id.toString())) {
                    ...
                    databaseFunctionManager.update_Member_Count_Database(
                      guild_Id,
                      user_Id,
                      allMembers_Count,
                      usersCount,
                      bots_Count,
                      allOnline_Count,
                      allOnline_Count_Idle_included,
                      allOnline_Count_Idle_Dnd_included,
                      allOffline_Count,
                      userOnline_Count,
                      userOnline_Count_Idle_included,
                      userOnline_Count_Idle_Dnd_included,
                      userIdle_Count,
                      userDnd_Count,


                      // @ts-expect-error TS(2554): Expected 14 arguments, but got 15.
                      userOffline_Count
                    );
                    await new Promise(async (resolve, reject) => {
                      ...
                      const parent = await createChannel(
                        interaction,
                        "Category",
                        "categoryChannel",
                        category_permissionOverwrites,
                        null
                      );
                      ...
                      // @ts-expect-error TS(2551): Property 'Guild_Id' does not exist on type 'Server... Remove this comment to see the full error message
                          ...
                          switch (value) {
                            case "0":
                              ...
                              serverStatusIds_Constructor.All_Members_Count_Id =
                                allMembers_Count_Channel.id.toString();
                              console.log("case 0 done");
                              break;
                            case "1":
                              let users_Count_Channel = await createChannel(
                                interaction,
                              ...
                              serverStatusIds_Constructor.Users_Count_Id =
                                users_Count_Channel.id.toString();
                              console.log("case 1 done");
                              break;
                            case "2":
                              let bots_Count_Channel = await createChannel(
                                interaction,
                              ...
                              channelFunctionManager.createChannel(
                                interaction,
                                "Bots_Count",
                                parent,
                                bots_Count
                              ...
                              serverStatusIds_Constructor.Bots_Count_Id =
                                bots_Count_Channel.id.toString();
                              console.log("case 2 done");
                              break;
                            case "3":
                              let allOnline_Count_Channel = await createChannel(
                                interaction,
                              ...
                              serverStatusIds_Constructor.All_Online_Count_Id =
                                allOnline_Count_Channel.id.toString();
                              console.log("case 3 done");
                              break;
                            case "4":
                              let allOnline_Count_Idle_included_Channel =
                                await createChannel(
                                  interaction,
                                  `All_Online_Count_include_idle:${allOnline_Count_Idle_included}`,
                                  "textChannel",
                                  permissionOverwrites,
                                  parent
                                );
                              serverStatusIds_Constructor.All_Online_Count_include_idle_Id =
                                allOnline_Count_Idle_included_Channel.id.toString();
                              console.log("case 4 done");
                              break;
                            case "5":
                              let allOnline_Count_Idle_Dnd_included_Channel =
                                await createChannel(
                                  interaction,
                                  `All_Online_Count_include_idle_dnd:${allOnline_Count_Idle_Dnd_included}`,
                                  "textChannel",
                                  permissionOverwrites,
                                  parent
                                );
                              serverStatusIds_Constructor.All_Online_Count_include_idle_dnd_Id =
                                allOnline_Count_Idle_Dnd_included_Channel.id.toString();
                              console.log("case 5 done");
                              break;
                            case "6":
                              let allOffline_Count_Channel =
                                await createChannel(
                                  interaction,
                              ...
                              serverStatusIds_Constructor.All_Offline_Count_Id =
                                allOffline_Count_Channel.id.toString();
                              console.log("case 6 done");
                              break;
                            case "7":
                              let userOnline_Count_Channel =
                                await createChannel(
                                  interaction,
                                  `Users_Online_Count:${userOnline_Count}`,
                                  "textChannel",
                                  permissionOverwrites,
                                  parent
                                );
                              serverStatusIds_Constructor.User_Online_Count_Id =
                                userOnline_Count_Channel.id.toString();
                              console.log("case 7 done");
                              break;
                            case "8":
                              let userOnline_Count_Idle_included_Channel =
                                await createChannel(
                                  interaction,
                                  `Users_Online_Count_include_idle:${userOnline_Count_Idle_included}`,
                                  "textChannel",
                                  permissionOverwrites,
                                  parent
                                );
                              serverStatusIds_Constructor.User_Online_Count_include_idle_Id =
                                userOnline_Count_Idle_included_Channel.id.toString();
                              console.log("case 8 done");
                              break;
                            case "9":
                              let userOnline_Count_Idle_Dnd_included_Channel =
                                await createChannel(
                                  interaction,
                                  `Users_Online_Count_include_idle_dnd:${userOnline_Count_Idle_Dnd_included}`,
                                  "textChannel",
                                  permissionOverwrites,
                                  parent
                                );
                              serverStatusIds_Constructor.User_Online_Count_include_idle_dnd_Id =
                                userOnline_Count_Idle_Dnd_included_Channel.id.toString();
                              console.log("case 9 done");
                              break;
                            case "10":
                              let userIdle_Count_Channel = await createChannel(
                                interaction,
                                `Users_Idle_Count:${userIdle_Count}`,
                                "textChannel",
                                permissionOverwrites,
                                parent
                              );
                              serverStatusIds_Constructor.User_Idle_Count_Id =
                                userIdle_Count_Channel.id.toString();
                              console.log("case 10 done");
                              break;
                            case "11":
                              let userDnd_Count_Channel = await createChannel(
                                interaction,
                                `Users_Dnd_Count:${userDnd_Count}`,
                                "textChannel",
                                permissionOverwrites,
                                parent
                              );
                              ...
                              // @ts-expect-error TS(2551): Property 'User_Dnd_Count_Id' does not exist on typ... Remove this comment to see the full error message
                              serverStatusIds_Constructor.User_Dnd_Count_Id =
                                userDnd_Count_Channel.id.toString();
                              console.log("case 11 done");
                              break;
                            case "12":
                              let userOffline_Count_Channel =
                                await createChannel(
                                  interaction,
                                  `Users_Offline_Count:${userOffline_Count}`,
                                  "textChannel",
                                  permissionOverwrites,
                                  parent
                                );
                              serverStatusIds_Constructor.User_Offline_Count_Id =
                                userOffline_Count_Channel.id.toString();
              ...
              await i
                .followUp({
                  content: "Please select [Status] first !",
                  ephemeral: false
                })
                .then((message) => {
                  setTimeout(() => {
