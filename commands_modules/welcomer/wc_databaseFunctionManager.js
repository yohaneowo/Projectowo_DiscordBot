...
class Welcomer_DatabaseManager {
    ...
    getWelcomer_Collection(Guild_Id) {
        return new Promise((resolve, reject) => {
            const db = new sqlite3.Database('./lib/database/SQLite.db')
            db.all(`SELECT * FROM Welcomer_Collection WHERE Guild_Id = ?`, [Guild_Id],function(err, row) {
                db.close()
                if (err) {
                    console.error(err.message);
                    reject(err);
    ...
    insertWelcomer_Collection(SetChannel_Id,Guild_Id, CreateAt, CreateBy) {
        return new Promise((resolve, reject) => {
            const db = new sqlite3.Database('./lib/database/SQLite.db')
            db.run(`INSERT INTO Welcomer_Collection VALUES (?,?,?,?,?)`, [null, SetChannel_Id, Guild_Id, CreateAt, CreateBy], function(err) {
                db.close()
                if (err) {
                    console.error(err.message);
                    reject(err);
    ...
    deleteWelcomer_Collection(Guild_Id) {
        return new Promise((resolve, reject) => {
            const db = new sqlite3.Database('./lib/database/SQLite.db')
            db.run(`DELETE FROM Welcomer_Collection WHERE Guild_Id = ?`, [Guild_Id], function(err) {
                db.close()
                if (err) {
                    console.error(err.message);
                    reject(err);
