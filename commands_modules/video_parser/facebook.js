...
const facebook_parser = async (message, URL) => {
  ...
  const messages = await PythonShell.run(
    "./python_script/yt-dlp/youtube-dl.py",
    options
  )
  ...
  let video_relativePath = `./lib/video/facebook/${fileName}`
  ...
  fs.stat(filePath, async (err, stats) => {
    ...
    if (fileSizeInMB < 25) {
      message.channel.send({
        ...
        files: [`./lib/video/facebook/${fileName}`]
      })
      wait_message.delete()
    } else {
      wait_message.delete()
      ...
      if (status === "done") {
        ...
        if (fs.existsSync(filePath)) {
          ...
          try {
            ...
            fs.unlinkSync(`./lib/video/${fileName}`)
