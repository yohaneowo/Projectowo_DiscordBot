    ...
    async execute(member) {
        ...
        if(guild_Ids.includes(member.guild.id)) {
            try {
                ...
                const Welcomer_row = await welcomer_DatabaseManager.getWelcomer_Collection(member.guild.id)
                member.guild.channels.fetch(Welcomer_row[0].Set_Channel).then(async channel => { 
                    ...
                    await ctx.drawImage(circleCanvas, circleX, circleY, radius * 2, radius * 2);
